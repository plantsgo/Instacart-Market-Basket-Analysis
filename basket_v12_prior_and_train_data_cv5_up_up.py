...
#标签
label=train[['order_id', 'product_id','reordered']].copy()
label.columns=['order_id', 'product_id','label']
'''
label_2=pd.DataFrame(label.groupby("order_id").label.sum()).reset_index()
label_2.columns=["order_id","label"]
label_2["product_id"]=49689
label_2["label"]=label_2["label"].apply(lambda x:1 if x==0 else 0)
label=label.append(label_2)
'''
...
print(train_x.shape)
train_x.head(10000).to_csv("train_x.csv",index=None)
#train_x.to_csv("train_data.csv",index=None)
#train_x.corr().to_csv("corr.csv")
"""
...
#train_x.to_csv("train.csv",index=None)
#test_data.to_csv("test.csv",index=None)
#train_x["orders_people"]=normalize(train_x["orders_people"])
#train_x["orders"]=normalize(train_x["orders"])
#test_data["orders_people"]=normalize(test_data["orders_people"])
#test_data["orders"]=normalize(test_data["orders"])
...
for tr_index,te_index in kf:
    ...
    """
    score_data = get_result(score_data)
    score_data.columns = ['order_id', 'products_pre']
    pre = score_data.sort_values("order_id")

    score = true.merge(pre, on="order_id", how="left")
    score["f1"] = list(chain(map(lambda a, b: eval_fun(a, b), score["products"], score["products_pre"])))
    print(score["f1"].mean())
    f1_score=score["f1"].mean()
    logloss=log_loss(train_y[te_index],pre_score)
    logloss_list.append(logloss)
